...
This is the light variant of two variants, mapped from the two sides of the
`darkTheme ? ... : ...` palette in `main.qml`. The other is `Althyn-Dark`
...
```sh
mkdir -p ~/.themes
cp -r Althyn-Light ~/.themes/
```

For Openbox you can instead double-click `Althyn-Light.obt`, or:

```sh
obconf --install Althyn-Light.obt
```
...
```sh
cd ~/.themes/Althyn-Light/openbox-3
